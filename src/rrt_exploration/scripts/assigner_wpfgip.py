...
def node():
    ...
    charging_stations = [
        ...
        array([1.323289, 1.5467890]),   # New station 1
        array([-2.9023, 1.78954]),      # New station 2
        array([5.89023, -2.78954]),
        array([2.48941028, -3.56222975]),
        array([1.31414, -1.31413])      # New station 3
    ]
    is_charging = False
    ...
    for i in range(0,n_robots):
        ...
        robots[i].assigned_point = None  # Initialize assigned_point as None
    ...
    while not rospy.is_shutdown():
        ...
        if is_charging:
            if time() - charge_start_time >= charging_time:
                ...
                # Resume exploration logic
                if last_exploration_point is not None:
                    rospy.loginfo("Returning to last exploration point: %s" % str(last_exploration_point))
                    for i in range(n_robots):
                        robots[i].sendGoal(last_exploration_point)
                        robots[i].assigned_point = None  # Reset assigned point
                    
                    # Wait for robot to reach last exploration point
                    timeout = time() + 100
                    reached_point = False
                    while not rospy.is_shutdown() and not reached_point:
                        ...
                        if all_robots_reached:
                            reached_point = True
                            rospy.loginfo("All robots reached last exploration point")
                            # Restore frontiers and recalculate centroids
                            frontiers = stored_frontiers
                            centroids = copy(frontiers)
                            
                            # Recalculate information gain for new centroids
                            infoGain = []
                            for ip in range(0, len(centroids)):
                                infoGain.append(informationGain(mapData, 
                                    [centroids[ip][0], centroids[ip][1]], info_radius))
                            
                            rospy.loginfo("Restored %d frontiers and ready to resume exploration" 
                                        % len(frontiers))
                            break
                            
                        if time() > timeout:
                            rospy.logwarn("Timeout reaching last exploration point!")
                            break
                            
                        rate.sleep()
                else:
                    rospy.logwarn("No last exploration point stored!")
                    # If no last point, just restore frontiers and continue
                    frontiers = stored_frontiers
                    centroids = copy(frontiers)
        ...
        if battery_level <= min_battery and not is_charging:
            ...
            for i in range(n_robots):
                ...
                while not rospy.is_shutdown():
                    ...
                    if time() > timeout:
                        rospy.logwarn("Robot %d timed out trying to reach charging station!" % i)
                        break
        ...
        for i in nb+na:
            if robots[i].assigned_point is not None:
                ...
                rospy.logdebug("Robot %d has no assigned point, skipping discount" % i)
        ...
        if len(na)<1:
            ...
            for ir in nb:
                for ip in range(0,len(centroids)):
                    ...
                    if robots[ir].assigned_point is not None and ((norm(centroids[ip]-robots[ir].assigned_point))<hysteresis_radius):
        ...
        if (len(id_record)>0):
            ...
            robots[id_record[winner_id]].assigned_point = centroid_record[winner_id]  # Update assigned point
