    ...
    while not rospy.is_shutdown():
        ...
        if is_charging:
            if time() - charge_start_time >= charging_time:
                ...
                # Reset robots to continue exploration
                for i in range(n_robots):
                    robots[i].sendGoal(robots[i].getPosition())  # Reset current position as goal
                    rospy.loginfo("Robot %d resuming exploration" % i)
            else:
        ...
        if battery_level <= min_battery and not is_charging:
            ...
            for i in range(n_robots):
                ...
                # Wait until robot reaches charging station
