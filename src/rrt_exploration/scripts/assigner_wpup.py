    ...
    while not rospy.is_shutdown():
        ...
        if battery_level <= min_battery and not is_charging:
            ...
            for i in range(n_robots):
                ...
                # Monitor progress with less frequent checks
                attempts = 0
                while not rospy.is_shutdown() and attempts < 100:  # Add reasonable attempt limit
                    current_pos = robots[i].getPosition()
                    ...
                    attempts += 1
                    rospy.sleep(0.5)  # Give more time between checks
                    rate.sleep()
            
            if is_charging:
                rospy.loginfo("Successfully started charging")
            else:
                rospy.logwarn("Could not reach charging station, will try again")
                robots[i].sendGoal(nearest_station)
