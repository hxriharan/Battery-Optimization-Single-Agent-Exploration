...
def node():
    ...
    min_battery = 97
    charging_time = 5.0
    charging_station = array([-1.280000, 1.610000])
    ...
    while not rospy.is_shutdown():
        ...
        for i in range(n_robots):
            ...
            # Add safety check for battery level
            robot_battery_levels[i] = max(0, min(100, robot_battery_levels[i]))  # Keep battery between 0-100%
            ...
            if robot_battery_levels[i] <= min_battery and not robot_charging_states[i]:
                ...
                if current_time - charging_start_times[i] > charging_timeout:
                    rospy.logwarn("Robot %d: Failed to reach charging station in time!" % i)
                    # Handle timeout - maybe try different charging station or emergency stop
                    continue
