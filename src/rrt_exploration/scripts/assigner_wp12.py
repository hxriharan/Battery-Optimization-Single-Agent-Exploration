    ...
    while not rospy.is_shutdown():
        ...
        if battery_level <= min_battery and not is_charging:
            ...
            for i in range(n_robots):
                ...
                while not rospy.is_shutdown() and not reached_station:
                    current_pos = robots[i].getPosition()
                    ...
                    if distance_to_station <= 0.5:
                        ...
                        # If not at station yet, make sure goal is still active
                        robot_state = robots[i].getState()
                        if robot_state != 1:  # If not actively moving
                            rospy.loginfo("Reinforcing goal for robot %d (state: %d)" % (i, robot_state))
                            robots[i].sendGoal(nearest_station)  # Resend goal
            ...
            if is_charging:
                ...
                rospy.logwarn("Still trying to reach charging station...")
