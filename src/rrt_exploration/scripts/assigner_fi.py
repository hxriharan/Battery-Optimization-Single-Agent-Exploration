...
def node():
    ...
    charging_stations = [
        ...
        array([1.1, 1.1]),              # New station 1
        array([-1.5, 1.5]),             # New station 2
        array([2.324568, -1.589872])              # New station 3
    ]
    is_charging = False
        ...
        if is_charging:
            if time() - charge_start_time >= charging_time:
                ...
                if len(frontiers) > 0:
                    ...
                    # Discount already explored areas more heavily
                    for i in nb + na:
                        current_pos = robots[i].getPosition()
                        if current_pos is not None:
                            for ip in range(len(centroids)):
                                distance_to_explored = norm(current_pos - centroids[ip])
                                if distance_to_explored < hysteresis_radius:
                                    infoGain[ip] *= 0.1  # Heavily discount already explored areas
                    
                    revenue_record = []
                    centroid_record = []
                    id_record = []
                    
                    # Calculate revenues with preference for unexplored areas
                    for ir in na:
                        current_pos = robots[ir].getPosition()
                        if current_pos is not None:
                            for ip in range(0, len(centroids)):
                                cost = norm(current_pos - centroids[ip])
                                information_gain = infoGain[ip]
                                
                                # Boost information gain for unexplored areas
                                if norm(current_pos - centroids[ip]) > hysteresis_radius:
                                    information_gain *= 2.0  # Boost unexplored areas
                    ...
                    # Assign frontiers to robots, prioritizing new areas
                    if len(revenue_record) > 0:
                        winner_id = revenue_record.index(max(revenue_record))
                        target_point = centroid_record[winner_id]
                        
                        # Verify this is a new area
                        robot_id = id_record[winner_id]
                        current_pos = robots[robot_id].getPosition()
                        if current_pos is not None:
                            rospy.loginfo("Assigning robot to new frontier. Distance from current: %.2f" % 
                                        norm(current_pos - target_point))
                        ...
                        rospy.loginfo(namespace + str(namespace_init_count + id_record[winner_id]) + 
                                    " assigned to new frontier at " + str(target_point))
                        rospy.sleep(delay_after_assignement)
                    
                    # Clear stored frontiers to force new frontier detection
                    stored_frontiers = []
                    last_exploration_point = None
