    ...
    while not rospy.is_shutdown():
        ...
        if is_charging:
            if time() - charge_start_time >= charging_time:
                ...
                # Return to last exploration point and resume
                if last_exploration_point is not None:
                    ...
                    frontiers = stored_frontiers  # Restore stored frontiers
                    rospy.loginfo("Restored %d stored frontiers" % len(stored_frontiers))
                    ...
                    while not rospy.is_shutdown():
                        for i in range(n_robots):
                            ...
                            if distance_to_point <= 0.5:
                                ...
                                # Immediately start exploring new frontiers
                                if len(frontiers) > 0:
                                    ...
                                    # Assign new frontier point
                                    revenue_record = []
                                    centroid_record = []
                                    id_record = []
                                    
                                    # Calculate revenues for each frontier
                                    for ip in range(0, len(centroids)):
                                    ...
                                    if len(revenue_record) > 0:
                                        ...
                                        rospy.loginfo("Robot %d assigned to new frontier at %s" % 
                                                    (id_record[winner_id], str(centroid_record[winner_id])))
                                break
                            
                            rate.sleep()
                    
                    # Reset exploration variables but keep frontiers
                    last_exploration_point = None
                    stored_frontiers = []  # Clear stored frontiers to allow new ones
                    
                else:
                    rospy.logwarn("No last exploration point stored!")
                    # Even if no last point, reset robot for new exploration
                    for i in range(n_robots):
                        robots[i].assigned_point = None
                        if len(frontiers) > 0:
                            rospy.loginfo("Starting new frontier exploration...")
                            centroids = copy(frontiers)
                            for ip in range(0, len(centroids)):
                                infoGain = []
                                infoGain.append(informationGain(mapData, [centroids[ip][0], centroids[ip][1]], info_radius))
